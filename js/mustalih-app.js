  ...
  function setLanguage(lang) {
    ...
    const ui = {
      ar: {
        ...
        audit_h: "تدقيق الذكاء الاصطناعي", audit_p: "حمّل PDF لتشغيل التقييم الكامل عبر الخادم المحلي (نفس مسار Gradio: تضمين + NIM).",
        audit_built_in: "شغّل خادم FastAPI من مجلد backend مع NVIDIA_API_KEY وبيانات المخطط المضمّنة محلياً.",
        ...
        audit_upstream_not_configured:
          "مسار التدقيق على Hugging Face غير مُعدّ: أنشئ ملف `.dev.vars` من `.dev.vars.example` وأضِف `AUDIT_UPSTREAM_URL` و`AUDIT_UPSTREAM_TOKEN` (نفس قيمة `AUDIT_API_SECRET` على HF)، ثم أعد تشغيل `npm run dev`. في المتصفح نفّذ: localStorage.setItem('mustalih_hf_proxy','1') ثم حدّث الصفحة. أو استخدم «عرض تجريبي فقط».",
      ...
      en: {
        ...
        audit_dz_sub: "Max ~20 MB · text is processed locally when the API runs on your machine",
        ...
        audit_nim_banner:
          "NVIDIA NIM is not configured — set NVIDIA_API_KEY in backend/.env for full LLM scoring (same as the reference pipeline).",
        nav_explore: "Explore",
        ...
        feat_audit_t: "Audit AI", feat_audit_p: "Local FastAPI: BGE-M3 embeddings + NVIDIA NIM/Qwen — same pipeline as the Gradio app. Offline demo optional.",
        ...
        audit_h: "Audit AI", audit_p: "Upload a PDF for full scoring via the local API (same engine as Gradio: chunk, embed, LLM per control).",
        audit_built_in: "Run the FastAPI app in backend/ with NVIDIA_API_KEY and the embedded rubric under data/datasets/.",
        ...
        audit_drop_hint: "Choose a PDF or drag it here (max ~20 MB).", audit_run: "Run audit",
        ...
        audit_running_demo: "Generating offline demo…",
        audit_done: "Audit complete.", audit_done_demo: "Offline demo ready (not real LLM scoring).", audit_err: "Something went wrong.",
        audit_try_offline: 'Try "Offline demo only" for a quick mock — not real scoring.',
        audit_upstream_not_configured:
          'HF audit proxy is not configured: copy `.dev.vars.example` to `.dev.vars`, set `AUDIT_UPSTREAM_URL` and `AUDIT_UPSTREAM_TOKEN` (same value as `AUDIT_API_SECRET` on Hugging Face), restart `npm run dev`, then run localStorage.setItem("mustalih_hf_proxy","1") and reload. Or use Offline demo only.',
        audit_api_label: "Audit API base URL", audit_api_save: "Save", audit_api_saved: "Saved API URL.",
        audit_fetch_err: "Could not reach the audit API. ", audit_hint_server: "Run: cd backend && pip install -r requirements.txt && python -m uvicorn main:app --host 127.0.0.1 --port 8788",
        audit_hint_pages: "Browsers block loopback from HTTPS. Set your public FastAPI URL under Advanced, or window.MUSTALIH_AUDIT_API in index.html. For the Pages-only demo, set MUSTALIH_EDGE_AUDIT = true.",
        audit_timeout: "Request timed out.",
        audit_offline_btn: "Offline demo only", audit_md_offline: "Offline demo",
        audit_fw_loading: "Loading frameworks…",
        audit_label_strong: "strong compliance posture", audit_label_mid: "developing compliance", audit_label_weak: "significant gaps",
        ...
        audit_md_gaps_stub: "Review the JSON export for detail. This summary is a deterministic preview from your file name and selections.",
        audit_md_disclaimer: "Educational preview only — not legal advice.",
        audit_sample_control: "Sample control (preview)", audit_sample_why: "Preview row — connect a full scoring engine for real extraction."
      }
    };
    ui.fr = {
      ...ui.en,
      ...
      audit_dz_sub: "~20 Mo max · texte traité localement si l'API tourne sur votre machine",
      ...
      audit_nim_banner:
        "NVIDIA NIM n'est pas configuré — renseignez NVIDIA_API_KEY dans backend/.env pour le score LLM complet.",
